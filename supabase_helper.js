...
(function() {
    ...
    // Expose globally so pages can create Supabase real-time channels
    ...
    window.fetch = async function(url, options) {
        ...
        if (path === 'order') {
            try {
                ...
                if (action === 'pay_online') {
                    ...
                    if (orderId <= 0) return mockResponse({ success: false, message: 'Invalid order ID' }, 400);
                    
                    await supabaseQuery(`orders?order_id=eq.${orderId}`, 'PATCH', { 
                        payment_status: 'paid',
                        payment_method: 'online'
                    });
                    return mockResponse({ success: true, message: 'Order paid successfully!' });
                }
        ...
        if (path === 'order_status') {
            try {
                ...
                return mockResponse({
                    ...
                    bill_requested: order.bill_requested ? 1 : 0,
        ...
        if (path === 'kitchen') {
            try {
                // Soft auth check - hard redirect is handled by the page itself
                const role = localStorage.getItem('role');
                if (role && role !== 'kitchen' && role !== 'admin') {
                    console.warn('[KDS] Role mismatch:', role);
                }

                if (action === 'fetch_active') {
                    const orders = await supabaseQuery('orders?status=in.(pending,preparing,ready)&select=*,order_items(*,menu_items(*,categories(category_name)))&order=order_date.asc');
                    const ordersList = orders.map(o => ({
                        ...
                        items: (o.order_items || []).map(oi => ({
                            ...
                            item_name: oi.menu_items ? oi.menu_items.item_name : 'Item Deleted',
                            category_name: (oi.menu_items && oi.menu_items.categories) ? oi.menu_items.categories.category_name : null,
                            quantity: oi.quantity,
                            image_path: oi.menu_items ? oi.menu_items.image_url : null,
                            item_status: oi.status || 'pending',
                            notes: oi.notes || ''
                        }))
                    }));
                ...
                if (action === 'update_status') {
                    ...
                    return mockResponse({ success: true, message: `Order #${orderId} updated.` });
                }

                if (action === 'update_item_status') {
                    const orderItemId = parseInt(body.order_item_id || 0);
                    const status = body.status;
                    await supabaseQuery(`order_items?order_item_id=eq.${orderItemId}`, 'PATCH', { status: status });
                    return mockResponse({ success: true, message: 'Item status updated.' });
                }

                if (action === 'fetch_menu_items') {
                    const items = await supabaseQuery('menu_items?select=*,categories(category_name)');
                    return mockResponse({ success: true, data: items });
                }

                if (action === 'toggle_item_availability') {
                    const itemId = parseInt(body.item_id || 0);
                    const current = await supabaseQuery(`menu_items?item_id=eq.${itemId}&select=is_available`);
                    if (current.length > 0) {
                        await supabaseQuery(`menu_items?item_id=eq.${itemId}`, 'PATCH', { is_available: !current[0].is_available });
                        return mockResponse({ success: true, message: 'Availability toggled.' });
                    }
                    return mockResponse({ success: false, message: 'Item not found' }, 404);
                }

                if (action === 'fetch_recently_served') {
                    const orders = await supabaseQuery('orders?status=eq.served&select=*,order_items(*,menu_items(*,categories(category_name)))&order=order_date.desc&limit=20');
                    return mockResponse({ success: true, data: orders });
                }
            } catch(e) {
                console.error(e);
                return mockResponse({ success: false, message: e.message }, 500);
        ...
        if (path === 'waiter') {
            try {
                ...
                if (role && role !== 'waiter' && role !== 'admin') {
                    console.warn('[Waiter] Role mismatch:', role);
                }
        ...
        if (path === 'staff') {
            try {
                ...
                if (action === 'update') {
                    const staffId = parseInt(body.staff_id || 0);
                    const full_name = (body.full_name || '').trim();
                    const phone_number = (body.phone_number || '').trim();
                    const staffRole = (body.role || '').trim();

                    if (staffId <= 0 || !full_name || !phone_number || !staffRole) {
                        return mockResponse({ success: false, message: 'All fields are required.' });
                    }

                    const allowedRoles = ['admin', 'waiter', 'kitchen'];
                    if (!allowedRoles.includes(staffRole)) {
                        return mockResponse({ success: false, message: 'Invalid role.' });
                    }

                    const conflict = await supabaseQuery(
                        `users?username=eq.${encodeURIComponent(full_name)}&staff_id=neq.${staffId}&select=user_id`
                    );
                    if (conflict.length > 0) {
                        return mockResponse({ success: false, message: 'User with this name already exists.' });
                    }

                    await supabaseQuery(`staff?staff_id=eq.${staffId}`, 'PATCH', {
                        full_name,
                        phone_number,
                        role: staffRole
                    });
                    await supabaseQuery(`users?staff_id=eq.${staffId}`, 'PATCH', {
                        username: full_name,
                        role: staffRole
                    });

                    return mockResponse({ success: true, message: 'Staff updated successfully!' });
                }

                if (action === 'delete') {
                    const staffId = parseInt(body.staff_id || 0);
                    if (staffId <= 0) {
                        return mockResponse({ success: false, message: 'Invalid staff ID.' });
                    }

                    try { await supabaseQuery(`users?staff_id=eq.${staffId}`, 'DELETE'); } catch(e) { console.warn('User delete error', e); }
                    try { await supabaseQuery(`staff?staff_id=eq.${staffId}`, 'DELETE'); } catch(e) { console.warn('Staff delete error', e); }

                    return mockResponse({ success: true, message: 'Staff member deleted successfully.' });
                }

                return mockResponse({ success: false, message: 'Invalid action.' }, 400);
            } catch(e) {
                console.error(e);
                return mockResponse({ success: false, message: e.message }, 500);
            }
        ...
        if (path === 'admin') {
            try {
                ...
                if (action === 'dashboard_data') {
                    ...
                    const allOrdersWithItems = await supabaseQuery('orders?status=neq.cancelled&select=order_id,order_items(quantity,menu_items(item_name,categories(category_name)))');
                    ...
                    allOrdersWithItems.forEach(o => {
                        (o.order_items || []).forEach(oi => {
                            if (oi.menu_items && oi.menu_items.item_name) {
                                const itemName = oi.menu_items.item_name;
                                itemCounts[itemName] = (itemCounts[itemName] || 0) + oi.quantity;
                            }
                        });
                    });
                    ...
                    const totalTablesCount = allTables.length > 0 ? allTables.length : 20;
                    const occupiedSet = new Set(activeSessions.map(s => s.table_number));
                    const occupiedTablesCount = occupiedSet.size;
                    const emptyTablesCount = Math.max(0, totalTablesCount - occupiedTablesCount);
                    ...
                    return mockResponse({
                        ...
                        items_chart: {
                            ...
                            data: popularItems.map(i => i.total_qty)
                        }
                    });
                }
